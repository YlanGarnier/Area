...
func NewProvider() (*Provider, error) {
	...
	if len(clientId) == 0 {
		return nil, fmt.Errorf("LINKEDIN_CLIENT_ID is not set")
	}
	clientSecret := os.Getenv("LINKEDIN_CLIENT_SECRET")
	if len(clientSecret) == 0 {
		return nil, fmt.Errorf("LINKEDIN_CLIENT_SECRET is not set")
	}
	urlOauth := os.Getenv("LINKEDIN_OAUTH_URL")
	if len(urlOauth) == 0 {
		return nil, fmt.Errorf("LINKEDIN_OAUTH_URL is not set")
	}
	urlApi := os.Getenv("LINKEDIN_API_URL")
	if len(urlApi) == 0 {
		return nil, fmt.Errorf("LINKEDIN_API_URL is not set")
	}
	return &Provider{
		ClientID:     clientId,
...
func (p Provider) GetUserIdentifier(accessToken string) (string, error) {
	email, err := utils.GetFromJsonReq[string](p.UrlApi+"/userinfo", "GET", "", []utils.Header{
		{
			Key:   "Authorization",
...
func (p Provider) GetUserTokenCredentials(code string, redirectUri string, platform models.Platform) (providers.TokenCredentials, error) {
	...
	tokenCredentials, err := utils.GetFromJsonReq[providers.TokenCredentials](p.UrlOauth+"/accessToken", "POST", data.Encode(), []utils.Header{
		{
			Key:   "Content-Type",
